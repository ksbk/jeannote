...
def test_projects_list_filter_and_card_navigation(
    ...
    page = mobile_page
    project_factory(title="Test House", tags="housing")
    project_factory(title="Office Fitout", tags="workplace")

    open_page("/projects/")

    expect(page.get_by_role("heading", name="Projects", level=1)).to_be_visible()

    filter_nav = page.get_by_role("navigation", name="Filter projects by tag")
    ...
    expect(page).to_have_url(f"{app_url}/projects/?tag=housing")
